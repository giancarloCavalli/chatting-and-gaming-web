...
import { GAME_COMMAND } from "../types/game-command";
...
export const sendGameCommand = (clientSocket: Socket, userId: string, password: string, command: string): void => {
  const UDP_PORT: number = Number(process.env.UDP_PORT);
  const UDP_HOST: string | undefined = process.env.UDP_HOST
  ...
  if (!GAME_COMMAND.includes(command)) {
    throw `Invalid command. Please inform one of this: ${GAME_COMMAND.map(x => x)}`
  }

  const request = `SEND GAME ${userId}:${password}:${command}`
